...
string training_filename = "face_recognition_densenetssl_training.dat";
string default_filename = "face_recognition_densenet_model_v1.dat";
...
namespace model {
    ...
    using train_ssl = loss_barlow_twins<ssl_projector<densenet::def<relu, bn_con, 16>::backbone<8, 12, 6, 3, input_rgb_image_pair>>>;
    using feats_ssl = loss_metric<ssl_projector<densenet::def<relu, affine, 16>::backbone<8, 12, 6, 3, input_rgb_image>>>;
    using train_std = loss_metric<metric_projector<densenet::def<relu, bn_con, 16>::backbone<8, 12, 6, 3, input_rgb_image>>>;
    using feats_std = loss_metric<metric_projector<densenet::def<relu, affine, 16>::backbone<8, 12, 6, 3, input_rgb_image>>>;
}
...
void metric_training(const std::string& folders, const std::string& model_filename, const bool sync_file, const double learning_rate, const size_t batch_size, const double weight_decay, const double momentum, const long patience, const long freezing_level, std::vector<int>& gpus) {
    ...
    if (freezing_level != 0) {
        ...
        case 1:
            set_learning_rate_multipliers_range<4, 282>(net, 1e-6); // Only fine-tune the top (last) FC layer
            break;
        case 2:
            set_learning_rate_multipliers_range<77, 282>(net, 1e-6); // FC and n_1 layers
            break;
        case 3:
            set_learning_rate_multipliers_range<189, 282>(net, 1e-6); // FC and n_1_2 layers
            break;
        case 4:
            set_learning_rate_multipliers_range<247, 282>(net, 1e-6); // FC and n_1_2_3 layers
            break;
        default:
            set_learning_rate_multipliers_range<4, 282>(net, 1e-6);
        }
    }
